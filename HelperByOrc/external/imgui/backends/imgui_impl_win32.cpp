...
static void ImGui_ImplWin32_UpdateMouseData(ImGuiIO& io)
{
    ...
    // GTA/SA:MP often leaves focus on a child HWND while the D3D host remains bd->hWnd; strict equality
    // skipped OS fallback and could strand MousePos until the next WM_MOUSEMOVE edge case.
    const bool is_app_focused = (focused_window == bd->hWnd)
        || (focused_window != nullptr && ::IsChild(bd->hWnd, focused_window) != FALSE);
    if (is_app_focused)
    {
        ...
        // When MouseTrackedArea != 0 the backend can still leave a stale client MousePos (e.g. after WM_MOUSELEAVE quirks);
        // refresh from OS if the queued position is not valid.
        if (!io.WantSetMousePos && (bd->MouseTrackedArea == 0 || !ImGui::IsMousePosValid(&io.MousePos)))
        {
            POINT pos;
