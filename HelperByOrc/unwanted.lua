...
local function TextRaw(s) imgui.TextUnformatted(tostring(s or "")) end
local function TextWrappedRaw(s) imgui.PushTextWrapPos(0); imgui.TextUnformatted(tostring(s or "")); imgui.PopTextWrapPos() end
local function Tooltip(txt) if imgui.IsItemHovered() then imgui.BeginTooltip(); imgui.PushTextWrapPos(420); TextRaw(txt); imgui.PopTextWrapPos(); imgui.EndTooltip() end end
local function SmallBtn(label, tip) local p = imgui.SmallButton(label); if tip then Tooltip(tip) end; return p end
local function HelpMark(txt) imgui.SameLine(); imgui.TextUnformatted(okfa and fa.CIRCLE_INFO or "(i)"); Tooltip(txt) end
...
-- === Кодировки ===
local function to_cp1251(s) local ok, res = pcall(function() return u8:decode(s or "") end); if ok and type(res)=="string" then return res end; return tostring(s or "") end
local function to_utf8(s) local ok, res = pcall(function() return u8:encode(s or "") end); if ok and type(res)=="string" then return res end; return tostring(s or "") end
...
local function rotate_backups(path)
  ...
  if exists(b3) then os.remove(b3) end
  if exists(b2) then os.rename(b2, b3) end
  if exists(b1) then os.rename(b1, b2) end
  local data = read_file(path)
  if data then write_file(b1, data) end
end
...
-- === Утилиты ===
local function escape_lua_magic(s) return (s:gsub("([%%%^%$%(%)%.%[%]%*%+%-%?])", "%%%1")) end
local function rule_enabled(it) return it.enabled ~= false end
...
local function is_valid_pattern(pat_cp1251)
  local okL, msg = lint_pattern(pat_cp1251); if not okL then return false, msg end
  local ok1 = pcall(string.find, "", pat_cp1251)
  local ok2 = pcall(string.find, " ", pat_cp1251)
  if not ok1 and not ok2 then return false, "Ошибка синтаксиса" end
  return true
end
...
function M.should_ignore(text_cp1251)
  if not (cfg.settings and cfg.settings.enabled) then return false end
  if type(text_cp1251) ~= "string" or text_cp1251 == "" then return false end
...
function M.get_flat_patterns_cp1251()
  local out = {}
  for i=1,#compiled.flat_patterns_cp1251 do out[i] = compiled.flat_patterns_cp1251[i] end
  return out
end
...
local function apply_edit(idx)
  local st = editor[idx]; if not st then return end
...
local function swap_editor(i, j) editor[i], editor[j] = editor[j], editor[i] end
...
local function selected_indices()
  local idxs = {}
  for i,_ in pairs(selection) do if selection[i] then idxs[#idxs+1] = i end end
  table.sort(idxs)
  return idxs
end
...
local function set_all_enabled(value)
  for _, it in ipairs(cfg.ignore) do it.enabled = value and true or false end
  rebuild_cache(); save_cfg()
end
...
local function run_tester()
  ...
  last_match = nil
  if not (cfg.settings and cfg.settings.enabled) then return end
...
local function helper_generate(sample_utf8, opts)
  ...
  if cp == "" then return { exact="", generalized="" } end
...
local function draw_rule_row(idx, item)
  ...
  if imgui.Checkbox("##sel", sel) then selection[idx] = sel[0] and true or false end
  Tooltip("Выделить правило для массовых операций"); imgui.SameLine()

  local en = ffi.new("bool[1]", rule_enabled(item))
  if imgui.Checkbox("##en", en) then item.enabled = en[0] and true or false; M.save() end
...
function M.DrawWindow()
  ...
  Tooltip("Пример: [Информация] {ffffff}Вы получили $18.400 за отредактированое вами объявление.")
  -- настройки помощника
  imgui.TextUnformatted("Обобщать:"); imgui.SameLine()
  imgui.Checkbox("Якоря ^ $", hp_anchor); Tooltip("Добавляет ^ в начало и $ в конец — совпадение по всей строке целиком.")
  imgui.SameLine()
  imgui.Checkbox("Деньги $123", hp_money); Tooltip("Превращает «$18.400» в шаблон «\\%$%d[%d%.,]*».")
  imgui.SameLine()
  imgui.Checkbox("Числа", hp_numbers); Tooltip("Числа и числа с разделителями заменяются на «%d+».")
  imgui.SameLine()
  imgui.Checkbox("Время 12:34", hp_time); Tooltip("Фрагменты вида 12:34 заменяются на «%d%d:%d%d».")
  imgui.SameLine()
  imgui.Checkbox("Цвета {HEX}", hp_colors); Tooltip("Фрагменты вида {ffffff} заменяются на «{%x%x%x%x%x%x}».")
  imgui.SameLine()
  imgui.Checkbox("Ник Имя_Фамилия", hp_nick); Tooltip("Фрагменты вида Имя_Фамилия заменяются на «%w+_%w+».")
  imgui.SameLine()
