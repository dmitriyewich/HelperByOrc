...
local function rebuild_cache()
	...
	for i, it in ipairs(cfg.ignore) do
		...
		if kind == "literal" then
			local rec =
				{ s = txt_c, nocase = it.nocase and true or false, whole_word = it.whole_word and true or false }
...
local function draw_rule_row(idx, item)
	...
	if st and st.editing then
		...
		if
			SmallBtn(okfa and (fa.CHECK .. " Сохранить") or "[SAVE]", "Сохранить изменения")
		then
			apply_edit(idx)
		end
		imgui.SameLine()
		...
		if st.is_pattern then
			...
			TextRaw(
				ok and (okfa and (fa.CHECK .. " Валидно") or "[OK] Валидно")
					or (
						okfa and (fa.TRIANGLE_EXCLAMATION .. " Ошибка: " .. (msg or ""))
						or "[ERROR] " .. (msg or "")
					)
			)
		end
		...
		if err then
			TextWrappedRaw(
				(item.text or "")
					.. "   "
					.. (
						okfa and (fa.TRIANGLE_EXCLAMATION .. " Ошибка: " .. tostring(err))
		...
		if last_match and last_match.idx == idx then
			TextRaw(
				okfa and (fa.CHECK .. " ← совпадение (тестер)")
					or "<- совпадение (тестер)"
			)
		end
	end
...
function M.DrawWindow(inline)
	...
	imgui.Separator()
	TextRaw(
		I(fa.WAND_MAGIC, "Автопомощник шаблонов (введите пример сообщения)")
	)
