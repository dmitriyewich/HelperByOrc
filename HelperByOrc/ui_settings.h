...
#define APP_UI_TEXTS(X) \
    ...
    X(TagsBuiltinArzDialogGetDialogTextFunctionDescription, "Возвращает токен текста последнего Arizona CEF диалога из RPC 61 по 0-based индексу. Разбиение такое же, как у [dialogtext(...)]. По плюсику рядом с тегом можно открыть список доступных индексов.", "Returns a token from the last Arizona CEF dialog text from RPC 61 by 0-based index. Splitting matches [dialogtext(...)]. Use the plus button next to the tag to open the list of available indexes.") \
