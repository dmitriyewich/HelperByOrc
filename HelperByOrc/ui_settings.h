...
#define APP_UI_TEXTS(X) \
    ...
    X(TagsBuiltinDialogWaitOpenDescription, "Ничего не вставляет в текст и дожидается появления активного диалога SA:MP до 3 секунд. После [ARZdialogitem(...)] в том же выполняющемся бинде ожидает следующий Arizona CEF диалог, даже если предыдущий ещё активен. Если диалог не открылся, текущий бинд будет остановлен.", "Inserts no text and waits up to 3 seconds for an active SA:MP dialog to appear. After [ARZdialogitem(...)] in the same bind, it waits for the next Arizona CEF dialog even while the previous one is still active. If no dialog opens, the current bind is stopped.") \
