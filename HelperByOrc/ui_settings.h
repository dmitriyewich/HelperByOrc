...
#define APP_UI_TEXTS(X) \
    ...
    X(TagsBuiltinHitMeNameDescription, "Возвращает имя из ника игрока, который последним нанёс вам урон по исходящему damage RPC.", "Returns the first name from the nickname of the player who last damaged you according to the outgoing damage RPC.") \
    X(TagsBuiltinHitMeSurnameDescription, "Возвращает фамилию из ника игрока, который последним нанёс вам урон по исходящему damage RPC.", "Returns the surname from the nickname of the player who last damaged you according to the outgoing damage RPC.") \
    X(TagsBuiltinHitByMeIdDescription, "Возвращает ID игрока, которому вы последним сообщили нанесённый урон через исходящий SA:MP damage RPC. Состояние очищается при смене сессии.", "Returns the player ID to whom you last reported dealt damage through the outgoing SA:MP damage RPC. State is cleared when the session changes.") \
    X(TagsBuiltinHitByMeNameDescription, "Возвращает имя из ника игрока, которому вы последним нанесли урон по исходящему damage RPC.", "Returns the first name from the nickname of the player you last damaged according to the outgoing damage RPC.") \
    X(TagsBuiltinHitByMeSurnameDescription, "Возвращает фамилию из ника игрока, которому вы последним нанесли урон по исходящему damage RPC.", "Returns the surname from the nickname of the player you last damaged according to the outgoing damage RPC.") \
    X(TagsBuiltinMyWantedDescription, "Возвращает текущий уровень розыска локального игрока от 0 до 6.", "Returns the local player's current wanted level from 0 to 6.") \
    X(TagsBuiltinMyInteriorDescription, "Возвращает текущий GTA area/interior ID локального игрока.", "Returns the local player's current GTA area/interior ID.") \
    X(TagsBuiltinMyHeadingDescription, "Возвращает направление тела локального игрока в градусах от 0 до 359.9 с одним знаком после точки.", "Returns the local player's body heading in degrees from 0 to 359.9 with one decimal place.") \
    X(TagsBuiltinGameTimeDescription, "Возвращает внутриигровое время GTA в формате HH:MM.", "Returns the GTA in-game time formatted as HH:MM.") \
    X(TagsBuiltinWeatherIdDescription, "Возвращает ID визуально преобладающей погоды: старой до 50% перехода и новой после.", "Returns the visually dominant weather ID: the old ID below 50% transition and the new ID afterward.") \
    X(TagsBuiltinMyCarIdDescription, "Возвращает SA:MP ID транспорта локального игрока. Поиск кэшируется на 250 мс; вне транспорта или для не-SA:MP транспорта возвращается пустая строка.", "Returns the local player's SA:MP vehicle ID. Lookup is cached for 250 ms; outside a vehicle or for a non-SA:MP vehicle it returns an empty string.") \
    X(TagsBuiltinMyCarModelIdDescription, "Возвращает GTA model ID транспорта локального игрока.", "Returns the GTA model ID of the local player's vehicle.") \
    X(TagsBuiltinMyCarColor1Description, "Возвращает основной GTA color ID транспорта локального игрока.", "Returns the primary GTA color ID of the local player's vehicle.") \
    X(TagsBuiltinMyCarColor2Description, "Возвращает дополнительный GTA color ID транспорта локального игрока.", "Returns the secondary GTA color ID of the local player's vehicle.") \
    X(TagsBuiltinMyCarEngineDescription, "Возвращает 1, если двигатель транспорта локального игрока включён, иначе 0.", "Returns 1 when the local player's vehicle engine is on, otherwise 0.") \
    X(TagsBuiltinMyCarLightsDescription, "Возвращает 1, если фары транспорта локального игрока включены, иначе 0.", "Returns 1 when the local player's vehicle lights are on, otherwise 0.") \
    X(TagsBuiltinMyCarSirenOrAlarmDescription, "Возвращает 1, если у транспорта локального игрока активна сирена или сигнализация, иначе 0.", "Returns 1 when the local player's vehicle siren or alarm is active, otherwise 0.") \
    X(TagsBuiltinMyCarSeatDescription, "Возвращает место локального игрока в транспорте: 0 для водителя, 1-8 для пассажира.", "Returns the local player's vehicle seat: 0 for the driver and 1-8 for a passenger.") \
    ...
    X(TagsBuiltinArzDialogCloseWithButtonDescription, "Закрывает текущий Arizona CEF диалог указанной кнопкой через DOM: 1 = Enter/primary, 0 = Escape/secondary. Если интерфейс ещё монтирует нужную кнопку, действие неблокирующе ждёт её появления до 1,5 секунды.", "Closes the current Arizona CEF dialog through DOM: 1 = Enter/primary, 0 = Escape/secondary. If the UI is still mounting the intended button, the action waits non-blockingly for up to 1.5 seconds.") \
    ...
    X(TagsBuiltinScoreFunctionDescription, "Возвращает SA:MP score игрока по ID или точному нику. Параметр поддерживает вложенные pure-теги; недоступный игрок даёт пустую строку.", "Returns a player's SA:MP score by ID or exact nickname. The parameter supports nested pure tags; an unavailable player returns an empty string.") \
    X(TagsBuiltinDistanceFunctionDescription, "Возвращает текущую 3D-дистанцию до застримленного игрока по ID или точному нику с одним знаком после точки. Недоступный игрок даёт пустую строку.", "Returns the current 3D distance to a streamed player by ID or exact nickname with one decimal place. An unavailable player returns an empty string.") \
    X(TagsBuiltinStrUpperDescription, "Преобразует текст в верхний регистр с поддержкой UTF-8 и CP1251. Максимальный вход и результат — 4096 байт.", "Converts text to uppercase with UTF-8 and CP1251 support. Input and output are capped at 4096 bytes.") \
    X(TagsBuiltinTrimDescription, "Удаляет Unicode-пробелы только в начале и конце текста, сохраняя внутренние символы и исходную кодировку UTF-8/CP1251.", "Removes Unicode whitespace only from the beginning and end while preserving inner characters and the original UTF-8/CP1251 encoding.") \
    X(TagsBuiltinSubstrDescription, "Возвращает часть текста по Unicode-символам: [substr(text;start;length)]. start — 1-based, length необязателен и не может быть отрицательным.", "Returns a Unicode-code-point substring: [substr(text;start;length)]. start is 1-based; length is optional and cannot be negative.") \
    X(TagsBuiltinStrlenDescription, "Возвращает число Unicode-символов в тексте, а не количество байт. Поддерживает UTF-8 и CP1251.", "Returns the number of Unicode code points in text, not the byte count. Supports UTF-8 and CP1251.") \
    X(TagsBuiltinClipboardSetDescription, "Записывает раскрытый текст в Unicode-буфер обмена Windows и ничего не вставляет в результат. Максимум 4096 байт; в HUD, preview и pure-режиме не выполняется.", "Writes expanded text to the Windows Unicode clipboard and inserts nothing into the result. Capped at 4096 bytes; inactive in HUD, preview, and pure mode.") \
    ...
    X(TagsBuiltinWaitIfDescription, "Неблокирующе ждёт истинности условия и продолжает текущий бинд. Поддерживает состояния игры/SA:MP, игроков, транспорт, клавиши, сравнения, not/and/or и скобки.", "Waits non-blockingly until a condition becomes true, then continues the current bind. Supports game/SA:MP state, players, vehicles, keys, comparisons, not/and/or, and parentheses.") \
    ...
    X(TagsBuiltinDialogWaitOpenDescription, "Ничего не вставляет в текст и дожидается появления активного диалога SA:MP до 3 секунд. После успешного [ARZdialogitem(...)] или [ARZdialogclosewithbutton(...)] в том же выполняющемся бинде ожидает следующий Arizona CEF диалог, даже если предыдущий ещё активен. Если диалог не открылся, текущий бинд будет остановлен.", "Inserts no text and waits up to 3 seconds for an active SA:MP dialog to appear. After a successful [ARZdialogitem(...)] or [ARZdialogclosewithbutton(...)] in the same running bind, it waits for the next Arizona CEF dialog even while the previous one is still active. If no dialog opens, the current bind is stopped.") \
    ...
    X(VariablesBuildWaitIf, "Собрать условие ожидания", "Build wait condition") \
    X(MiscVariablesWaitIfBuilderOpenHint, "Выбрать состояние, игрока, сравнение или клавишу и получить готовую [waitif(...)].", "Choose a state, player, comparison, or key and generate a ready [waitif(...)].") \
    X(WaitIfBuilderTitle, "Конструктор [waitif(...)]", "[waitif(...)] builder") \
    X(WaitIfBuilderIntro, "Условие проверяется без блокировки игры и удерживает только текущую строку бинда. Можно выбрать готовое действие или ввести составное выражение вручную.", "The condition is checked without blocking the game and holds only the current bind line. Choose a predefined action or enter a compound expression manually.") \
    X(WaitIfBuilderRawMode, "Составное выражение вручную", "Manual compound expression") \
    X(WaitIfBuilderCategory, "Раздел", "Category") \
    X(WaitIfBuilderActionSearch, "Поиск по действию или описанию", "Search by action or description") \
    X(WaitIfBuilderActionSearchEmpty, "В этом разделе совпадений нет.", "No matching actions in this category.") \
    X(WaitIfBuilderAction, "Действие / значение", "Action / value") \
    X(WaitIfBuilderPlayer, "Игрок", "Player") \
    X(WaitIfBuilderCustomPlayer, "ID, ник или переменная игрока", "Player ID, nickname, or variable") \
    X(WaitIfBuilderComparison, "Сравнение", "Comparison") \
    X(WaitIfBuilderValue, "Значение", "Value") \
    X(WaitIfBuilderKey, "Клавиша", "Key") \
    X(WaitIfBuilderNegate, "Инвертировать условие (not)", "Negate condition (not)") \
    X(WaitIfBuilderExpression, "Выражение", "Expression") \
    X(WaitIfBuilderPreview, "Готовая переменная", "Generated variable") \
    X(WaitIfBuilderExpressionTooLong, "Выражение превышает лимит 4096 байт.", "The expression exceeds the 4096-byte limit.") \
    X(WaitIfBuilderRawHint, "Пример: not ChatinputActive and getCharHealth({myid}) < 100", "Example: not ChatinputActive and getCharHealth({myid}) < 100") \
