...
local function send_sequence(messages, method, interval)
	...
	if send_sequence_running then
		update_status(
			"Уже выполняется отправка сообщений. Дождитесь завершения текущей очереди."
		)
		return
	end
...
local function broadcast_winner_gender(player_name, score, player_id, gender, answer, points_awarded)
	...
	local template = gendered_messages
			...
			and gendered_messages[math_random(1, #gendered_messages)]
...
local function announce_latest_stats(gender)
	...
	if has_winner then
		...
		broadcast_correct_answer_gender(
			stats.winner,
			stats.correct_answer,
			stats.score,
			stats.game_finished,
...
local function parse_sms_message(text)
	...
	local pattern =
		"%[SМS на студию%]%s*{FFFFFF}%s*Слушатель:%s*{%x%x%x%x%x%x}([%w_]+)%[(%d+)%]%s*{FFFFFF}:%s*(.+)"
...
local function ensure_player(name)
	...
	MathQuiz.players[normalized] = MathQuiz.players[normalized]
		or { score = 0, last_answer = nil, last_correct = false }
...
local function iterate_players_sorted()
	...
	for name, data in pairs(MathQuiz.players) do
		table.insert(
			list,
			{ name = name, score = data.score, last_correct = data.last_correct, last_answer = data.last_answer }
		)
	end
...
local function compute_lead_time(first_response)
	...
	for _, resp in ipairs(MathQuiz.current_responses) do
		if
			...
			and first_response.response_time
			and resp.response_time > first_response.response_time
		then
...
local function record_response_from_sms(player_name, player_id, message)
	...
	if is_correct and not MathQuiz.first_correct then
		...
		if MathQuiz.active then
			if lead then
				update_status(
					"%s отвечает верно за %.2f с и опережает соперников на %.2f с. Счёт: %d/%d.",
					player_name,
					entry.response_time,
					lead,
					player_entry.score,
				...
				update_status(
					"%s отвечает верно за %.2f с. Других ответов пока нет. Счёт: %d/%d.",
					player_name,
					entry.response_time,
					player_entry.score,
					target
			...
			if lead then
				update_status(
					"%s завершает игру, ответив верно за %.2f с и опередив соперников на %.2f с. Итог: %d очков.",
					player_name,
					entry.response_time,
					lead,
					player_entry.score
				...
				update_status(
					"%s завершает игру, ответив верно за %.2f с. Итог: %d очков.",
					player_name,
					entry.response_time,
					player_entry.score
				)
	...
	if is_correct then
		...
		update_status(
			"%s прислал верный ответ через %.2f с, но уже после завершения раунда.",
			player_name,
			entry.response_time
		)
	else
...
local function draw_latest_round_stats()
	...
	local summary = string.format(
		"Первый верный: %s[%s] - %s | Всего ответов: %d",
		winner,
		player_id,
		response_time,
		total_responses
...
function SMILive.DrawMathQuiz(show_tables)
	...
	if not MathQuiz.active then
		...
		if MathQuiz.current_problem then
			...
			if MathQuiz.show_answer[0] and MathQuiz.current_answer then
				...
				imgui.TextColored(
					...
					string.format("= %s", tostring(MathQuiz.current_answer))
				)
			end
		end
		...
		if imgui.Button("Использовать пример") then
			if MathQuiz.awaiting_next_round then
				local message =
					'Следующий раунд начнётся после объявления победителя. Нажмите "Следующий пример".'
	...
	if MathQuiz.latest_round_stats and MathQuiz.latest_round_stats.winner then
		...
		local stats = MathQuiz.latest_round_stats
		local has_winner = MathQuiz.winner ~= nil
		local male_label = has_winner and "Объявить ответ и победителя (м)"
			or "Объявить ответ (м)"
		local female_label = has_winner and "Объявить ответ и победителя (ж)"
			or "Объявить ответ (ж)"
		...
		show_tooltip(
			string.format(
				"Отправить сообщение для игрока мужского пола, %s",
				display_name
			)
		)
		pop_button_palette()
		...
		show_tooltip(
			string.format(
				"Отправить сообщение для игрока женского пола, %s",
				display_name
			)
		)
		pop_button_palette()
