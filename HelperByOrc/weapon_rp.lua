...
local function draw_weapon_row(id, w)
	...
	dl:AddRectFilled(
		start,
		imgui.ImVec2(start.x + fullW, start.y + row_h),
	...
	imgui
		.GetWindowDrawList()
		:AddText(imgui.ImVec2(txt_x, txt_y), imgui.GetColorU32Vec4(imgui.GetStyle().Colors[imgui.Col.Text]), label)
	...
	if imgui.CollapsingHeader("Слова и связки (общие)") then
		...
		if
			imgui.InputTextMultiline(
				"Соединители (полные)",
				conn_full,
				ffi.sizeof(conn_full),
				imgui.ImVec2(0, 80)
			)
		then
			M.config.connectors_full = funcs.parseLines(ffi.string(conn_full), true)
			save_cfg()
		end
		...
		then
			M.config.connectors_short = funcs.parseLines(ffi.string(conn_short), true)
			save_cfg()
		end
	end
	...
	if imgui.CollapsingHeader("Оружие") then
		...
		if imgui.BeginPopup("weapon_add_popup") then
			...
			if imgui.Button("Текущее") then
				local ped = PLAYER_PED
				if ped then
					M._new_id[0] = getCurrentCharWeapon(ped) or 0
				end
			end
			...
			if imgui.Button("OK##addw") then
				...
				local show = funcs.parseList(ffi.string(M._new_show))
				if #show == 0 then
					show = { "достал" }
				end
				local hide = funcs.parseList(ffi.string(M._new_hide))
				if #hide == 0 then
					hide = { "убрал" }
				end
				if nm ~= "" then
			...
			imgui.SameLine()
			if imgui.Button("Отмена##addw") then
				imgui.CloseCurrentPopup()
			end
			imgui.EndPopup()
		end
