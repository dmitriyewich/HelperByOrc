...
-- ===================== JSON: БЕЗ КРАКОЗЯБР =====================
local function map_strings(tbl, fn)
    if type(tbl) ~= "table" then
        if type(tbl) == "string" then return fn(tbl) else return tbl end
    end
    local res = {}
    for k, v in pairs(tbl) do
        local nk = type(k) == "string" and fn(k) or k
        if type(v) == "table" then
            res[nk] = map_strings(v, fn)
...
local function save_json_utf8(path, tbl_cp1251)
    local tbl_utf8 = map_strings(tbl_cp1251, function(s) return u8:encode(s) end)
    local ok, encoded = pcall(encodeJson, tbl_utf8)
...
local function load_json_utf8(path)
    ...
    return map_strings(data, function(s) return u8:decode(s) end)
end
...
function module.load()
    ...
    local loaded = load_json_utf8(json_path)
...
function module.save()
    save_json_utf8(json_path, config)
end
...
function module.DrawSettingsWindow()
    ...
    if imgui.Begin("VIP/AD чат — настройки", settings_open) then
        ...
        if imgui.Button("Экспортировать конфиг") then
            local f = io.open(json_path..".backup", "wb")
            if f then
                local tmp_utf8 = map_strings(config, function(s) return u8:encode(s) end)
                f:write(encodeJson(tmp_utf8))
                f:close()
            end
        end
        imgui.SameLine()
        if imgui.Button("Импортировать из backup") then
            if doesFileExist(json_path..".backup") then
                local ok, tbl = pcall(function()
                    local f = io.open(json_path..".backup", "rb")
                    if not f then return nil end
                    local content = f:read("*a"); f:close()
                    local ok2, data = pcall(decodeJson, content)
