...
local function draw_text_with_highlight(text, highlightWordsLower, rect_color, text_alpha)
	...
	local ts_scale = clamp(
		tonumber(timestamp_cfg.scale) or (default_config.timestamp and default_config.timestamp.scale) or 0.5,
		0.2,
		1.0
	)
	local ts_padding = math.max(
		0.0,
		tonumber(timestamp_cfg.padding) or (default_config.timestamp and default_config.timestamp.padding) or 0.0
	)
...
imgui.OnFrame(function()
	return module.showFeedWindow[0] and config.enabled
end, function(VIPandADchat)
	...
	local window_flags, child_flags_mask, allow_interaction =
		compute_feed_interaction_state(is_chat_open, cursor_active)
	if not config then
		return
	end
	...
	if imgui.Begin("##VIPADFEED", module.showFeedWindow, flags) then
		...
		-- ===== VIP =====
		imgui.BeginChild("VIP", imgui.ImVec2(max_width, config.vip_height * lh + 4), true, child_flags)
		local vip = config.table_config.vip_text
		local vip_lines = #vip
		...
		-- ===== AD =====
		imgui.BeginChild("AD", imgui.ImVec2(max_width, config.ad_height * lh + 4), true, child_flags)
		local ad = config.table_config.ad_text
		local ad_lines = #ad
...
local function draw_settings_content()
	...
	if imgui.InputInt("X##posx", pos_x) then
		config.pos_x = math.max(0, pos_x[0])
		changed_basic = true
	end
	imgui.SameLine()
	if imgui.InputInt("Y##posy", pos_y) then
		config.pos_y = math.max(0, pos_y[0])
		changed_basic = true
	end
	imgui.SameLine()
	if imgui.InputInt("Ширина", width) then
		config.width = math.max(200, width[0])
		changed_basic = true
	end
	imgui.SameLine()
	if imgui.InputInt("VIP-строк", vip_h) then
		config.vip_height = math.max(1, vip_h[0])
		changed_basic = true
	end
	imgui.SameLine()
	if imgui.InputInt("AD-строк", ad_h) then
		config.ad_height = math.max(1, ad_h[0])
		changed_basic = true
	end
	imgui.PopItemWidth()

	if changed_basic and imgui.IsMouseReleased(0) then
		module.save()
	end
	...
	local ts_scale =
		ffi.new("float[1]", ts.scale or (default_config.timestamp and default_config.timestamp.scale) or 0.5)
	...
	for i, word in ipairs(config.highlightWords) do
		...
		if imgui.InputText("##w", buf, ffi.sizeof(buf)) then
			config.highlightWords[i] = ffi.string(buf)
			module.save()
		end
		imgui.SameLine()
		if imgui.SmallButton("Удалить##w" .. i) then
			table.remove(config.highlightWords, i)
			module.save()
		end
		imgui.PopID()
	end
	local newWord = imgui.new.char[64]()
	if imgui.InputText("Добавить ключевое слово", newWord, ffi.sizeof(newWord)) then
	end
	imgui.SameLine()
	if imgui.SmallButton("Добавить") and ffi.string(newWord) ~= "" then
		table.insert(config.highlightWords, ffi.string(newWord))
		module.save()
	end

	imgui.Separator()
	if imgui.Button("Очистить VIP-ленту") then
		module.ClearVIP()
	end
	imgui.SameLine()
	if imgui.Button("Очистить AD-ленту") then
		module.ClearAD()
	end
