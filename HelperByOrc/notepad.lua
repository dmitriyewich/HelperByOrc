...
local function loadTreeFromFiles()
    ...
    for _, path in ipairs(tree_flat) do
        ...
        for part in path:gmatch("([^\\]+)") do
            if not node[part] then node[part] = {} end
            node = node[part]
        end
        node.__is_file = path
    end
...
local function readTxtFile(path, callback)
    if txt_cache[path] then return callback(txt_cache[path]) end
...
local function addToHistory(idx)
    for i=#history,1,-1 do
        if history[i] == idx then table.remove(history, i) end
    end
...
local function isDuplicateTitle(title)
    for i, n in ipairs(notes) do
        if n.title == title then return true end
    end
    return false
end
local function createNote()
    ...
    table.insert(notes, {title = title, category = cat, text = "", _fav = false, _ctime = os.time(), _mtime = os.time()})
    saveNotes()
...
local function duplicateNote(idx)
    ...
    if note then
        ...
        table.insert(notes, {
            title = t, category = note.category, text = note.text, _fav = false, _ctime = os.time(), _mtime = os.time()
        })
        saveNotes()
    end
end
...
local function split_text_wrapped(text)
    ...
    while start <= #text do
        ...
        while text_ptr < text_end do
            ...
            if text_ptr[0] == 32 then text_ptr = text_ptr + 1 end
        end

        start = line_end + 1
        if line_end <= #text then table.insert(result, "") end
    end
    return result
end
...
local function searchAllNotesAndTxt(filterRaw)
    if not filterRaw or filterRaw == "" then return {} end
    local results = {}
    for i, note in ipairs(notes) do
        if passFilter(note.title, filterRaw) then
            table.insert(results, {idx=i, line_idx=0, line="", is_title=true, category=note.category, isJson=true})
        end
        local line_num = 1
        for line in (note.text or ""):gmatch("([^\n]*)\n?") do
            if passFilter(line, filterRaw) then
                table.insert(results, {idx=i, line_idx=line_num, line=line, is_title=false, category=note.category, isJson=true})
            end
            line_num = line_num + 1
        end
    end
    for _, path in ipairs(tree_flat) do
        ...
        if content then
            ...
            for line in content:gmatch("([^\n]*)\n?") do
                if passFilter(line, filterRaw) then
                    table.insert(results, {path=path, line_idx=line_num, line=line, is_title=false, isJson=false})
                end
                line_num = line_num + 1
            end
        end
...
local function drawCat(cat, arr, star)
    ...
    if imgui.BeginPopupContextItem(cat.."popupcat") then
        ...
        if imgui.MenuItemBool("Удалить все заметки в папке") then
            for i = #notes,1,-1 do
                if notes[i].category == cat then table.remove(notes, i) end
            end
            saveNotes()
        end
    ...
    if tree_open then
        ...
        while clipper2:Step() do
            for j = clipper2.DisplayStart + 1, clipper2.DisplayEnd do
                ...
                if isSel then imgui.PushStyleColor(imgui.Col.Header, imgui.ImVec4(0.2,0.52,1,1)) end
                imgui.SetCursorPosY(imgui.GetCursorPosY() + 2)
                if imgui.Selectable((star and star_icon or note_icon or "").." "..v.note.title, isSel, 0, imgui.ImVec2(0, imgui.GetFontSize()+8)) then
                ...
                if imgui.BeginPopupContextItem("note"..v.idx) then
                    ...
                    if imgui.MenuItemBool(v.note._fav and "Убрать из избранного" or "В избранное") then
...
local function drawLeftPanel()
    ...
    imgui.SameLine()
    if imgui.Button((plus_icon or "+").."##addnote", imgui.ImVec2(28, 22)) then createNote() end
    imgui.Spacing()
    imgui.Separator()
    ...
    if filter:IsActive() then
        if imgui.Button(close_icon or "[X]", imgui.ImVec2(25, 0)) then filter:Clear() end
    end
    imgui.PopItemWidth()
    imgui.Spacing()
    if star_icon ~= "" then imgui.Text(star_icon.." Последние") else imgui.Text("Последние") end
    ...
    while clipper:Step() do
        for i = clipper.DisplayStart + 1, clipper.DisplayEnd do
            ...
            if note and passFilter(note.title.." "..note.text, ffi.string(filter.InputBuf)) then
                ...
                imgui.SetCursorPosY(imgui.GetCursorPosY() + 2)
                if imgui.Selectable((note_icon or "").." "..note.title, isSel, 0, imgui.ImVec2(0, fixed_height)) then
                ...
                if imgui.BeginPopupContextItem("note"..idx) then
                    ...
                    if imgui.MenuItemBool(note._fav and "Убрать из избранного" or "В избранное") then
    ...
    for i, note in ipairs(notes) do
        if not passFilter(note.title.." "..note.text, ffi.string(filter.InputBuf)) then goto continue end
    ...
    for cat, arr in pairs(pinned) do drawCat(cat, arr, true) end
    for cat, arr in pairs(categories) do drawCat(cat, arr, false) end
    imgui.Spacing()
    imgui.Separator()
...
local function drawRightPanel()
    ...
    elseif selectedNode.isJson then
        ...
        if note then
            ...
            imgui.SameLine()
            if note._fav then imgui.TextColored(imgui.ImVec4(1,1,0.1,1), star_icon); imgui.SameLine() end
            if imgui.SmallButton(star_icon..(note._fav and " Открепить" or " Закрепить")) then note._fav = not note._fav; saveNotes() end
            imgui.SameLine()
            if imgui.SmallButton(exp_icon.." Экспорт") then exportNote(note) end
            imgui.SameLine()
            if imgui.SmallButton((copy_icon or "").." Скопировать") then copyToClipboard(note.text or "") end
            imgui.SameLine()
            ...
            imgui.SameLine()
            imgui.TextDisabled(os.date("Создано: %d.%m.%Y %H:%M", note._ctime).." | "..os.date("Изменено: %d.%m.%Y %H:%M", note._mtime))
            imgui.Spacing()
            if not editingMode[0] then
                imgui.Separator()
                if scrollToLine and scrollToLine > 0 then
                    for i=1,scrollToLine-1 do imgui.TextUnformatted("") end
                    scrollToLine = nil
                end
                ...
                imgui.Separator()
                local savePressed = imgui.Button((save_icon or "").." Сохранить   (Ctrl+S)") or (imgui.IsKeyDown(0x11) and imgui.IsKeyPressed(0x53))
                if savePressed then
                ...
                imgui.SameLine()
                if imgui.Button((cancel_icon or "").." Отмена") then editingMode[0] = false end
                imgui.SameLine()
                if imgui.Button((delete_icon or "").." Удалить") then table.remove(notes, idx); saveNotes(); selectedNode = nil; editingMode[0] = false; return end
                imgui.InputTextMultiline("##editnotetext", editingText, editingBufSize, imgui.ImVec2(-1, getDynamicTextHeight(ffi.string(editingText))))
        ...
        imgui.Text(name)
        imgui.SameLine()
        if imgui.SmallButton(exp_icon.." Экспорт") then exportNote({title = name, text = content}) end
        imgui.Separator()
        if not editingMode[0] then
            if scrollToLine and scrollToLine > 0 then
                for i=1,scrollToLine-1 do imgui.TextUnformatted("") end
            ...
            if imgui.Button((edit_icon or "").." Редактировать") then editingMode[0] = true end
        else
            imgui.Separator()
            if imgui.Button((save_icon or "").." Сохранить как новую") then
                table.insert(notes, {title=name, category="Импорт", text=content, _fav=false, _ctime=os.time(), _mtime=os.time()})
                saveNotes()
                editingMode[0] = false
            end
            imgui.SameLine()
            if imgui.Button((cancel_icon or "").." Отмена") then editingMode[0] = false; editingText = imgui.new.char[#content+2048](content) end
            imgui.InputTextMultiline("##editimptxt", editingText, editingBufSize, imgui.ImVec2(-1, getDynamicTextHeight(ffi.string(editingText))))
            imgui.TextDisabled(("Символов: %d"):format(#ffi.string(editingText)))
        end
    end
    ...
    if imgui.BeginPopup("moveNote") then
        ...
        imgui.SameLine()
        if imgui.Button("Отмена##movecat") then imgui.CloseCurrentPopup(); popup_move[0] = false end
        imgui.EndPopup()
    end
    if imgui.BeginPopup("longTextWarn") then
        imgui.TextColored(imgui.ImVec4(1,0.45,0.2,1), "Слишком длинный текст!\nОграничение: 65535 символов.")
        if imgui.Button("OK##longwarn") then imgui.CloseCurrentPopup(); popup_warning[0]=false end
        imgui.EndPopup()
    end
    ...
    if imgui.BeginPopupModal("RenamePopup", nil, imgui.WindowFlags.AlwaysAutoResize) then
        ...
        imgui.SameLine()
        if imgui.Button("Отмена##rename") then imgui.CloseCurrentPopup() end
        imgui.EndPopup()
    end
    ...
    if imgui.BeginPopupModal("NewFolderPopup", nil, imgui.WindowFlags.AlwaysAutoResize) then
        ...
        if imgui.Button("OK##newfolder") then
            ...
            table.insert(notes, {title = "Новая заметка", category = new_cat, text = "", _fav = false, _ctime = os.time(), _mtime = os.time()})
            saveNotes()
            imgui.CloseCurrentPopup()
        end
        imgui.SameLine()
        if imgui.Button("Отмена##newfolder") then imgui.CloseCurrentPopup() end
        imgui.EndPopup()
    end
    imgui.EndChild()
end
