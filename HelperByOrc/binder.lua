...
local function drawFolderTabs()
	...
	local function drawOpenPathBreadcrumbs(path)
		imgui.Text("Открыто: ")
		imgui.SameLine()
		local node = nil
		local style = imgui.GetStyle()
	...
	local openText = "Открыто: " .. table.concat(path, " / ")
	local style = imgui.GetStyle()
	local gap = 8
	imgui.SameLine(0, gap)
	local rightW = imgui.GetContentRegionAvail().x
