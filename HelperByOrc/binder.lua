...
local function drawEditHotkey(idx)
        ...
        if hk.editMultiline then
            ...
            for line in (hk.editMultiText or ""):gmatch("[^\r\n]+") do
                if line ~= "" then
                    table.insert(
                        hk.editMsgs,
                        { text = line, interval = hk.editBulkInterval or "0", method = hk.editBulkMethod or 0 }
                    )
                end
            end
            module.saveHotkeys()
    ...
    if hk.editMultiline then
        ...
        for i, m in ipairs(hk.editMsgs) do
            ...
            if
                imgui.InputText("Текст##t", tbuf, ffi.sizeof(tbuf), flags_or(imgui.InputTextFlags.AutoSelectAll))
            then
    ...
    local function drawFolderRow(folder, isRoot)
        ...
        for i, f in ipairs(list) do
            ...
            if imgui.BeginPopup("popup_gear_" .. tostring(f)) then
                ...
                for ii = 1, quick_cond_count do
                    ...
                    if
                        imgui.Checkbox(quick_cond_labels[ii] .. "##fq" .. ii .. tostring(f), f._quick_cond_bools[ii])
                    then
        ...
        if
            imgui.Button(
                ...
                imgui.ImVec2(tabHeight, tabHeight)
            )
        then
