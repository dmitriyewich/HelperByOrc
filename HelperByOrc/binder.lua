...
local function check_conditions(conds)
  local funcs_conds = {
  ...
  for idx, v in ipairs(conds or {}) do
    if v and funcs_conds[idx] and safe_call(funcs_conds[idx]) then
      return false
    end
  end
  return true
...
function module.findBind(name, folder)
  ...
  for _, hk in ipairs(hotkeys) do
    ...
    if folder and folder ~= "" then
      ...
      inFolder = fstr:find(tostring(folder):lower(), 1, true) and true or false
    end
    if inFolder and hk.label and hk.label:lower() == name then
      return hk
    end
  end
  for _, hk in ipairs(hotkeys) do
    ...
    if inFolder and hk.label and hk.label:lower():find(name, 1, true) then
      return hk
    end
  end
  return nil
