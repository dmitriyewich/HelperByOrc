...
local function load_external_vars()
  ...
  for _, path in ipairs(files) do
    ...
    if not chunk then
      ...
      if not ok then
        errors = errors + 1
        log_chat(("[Tags] Ошибка при выполнении '%s': %s"):format(path, tostring(perr)), 0xAA3333)
      else
        loaded = loaded + 1
...
local tags = setmetatable({}, {
  __index = function(_, key)
    ...
    elseif key == "{targetid}" then
      return function() return target.last_id and tostring(target.last_id) or "" end
...
local function get_custom_var_list()
  local out = {}
  for k, v in pairs(custom_vars) do out[#out + 1] = { key = k, value = v } end
...
local function get_func_list()
  ...
  for tag, v in pairs(multi_tags_descriptions) do
    table.insert(out, { name = ("[%s(...)]"):format(tag), desc = v.desc, example = v.example or ("[%s(...)]"):format(tag) })
  end
  table.sort(out, function(a, b) return a.name < b.name end)
  return out
end
...
function module.DrawSettingsPage()
  imgui.TextColored(imgui.ImVec4(0.75, 1, 1, 1), "Переменные и теги — удобно и без лишнего шума")
  imgui.Separator()
  ...
  if imgui.BeginTabBar("tags_tabbar") then
    ...
    if imgui.BeginTabItem("Основное") then
      ...
      do
        ...
        imgui.SameLine()
        Badge("{targetid}")
        HelpTip("При смене цели выводится всплывашка с ID. Удобно, если часто используешь теги по цели.")
      end

      -- Разрешить $call
      do
        local v = ffi.new("bool[1]", settings.allow_unsafe and true or false)
        if imgui.Checkbox("Разрешить $call (только если понимаешь риски)", v) then
          settings.allow_unsafe = v[0] and true or false
          save_config()
        end
        HelpTip("Выполнение Lua-выражений из строки. Включай только для своих шаблонов. На чужие не ставь.")
      end
      ...
      do
        ...
        HelpTip("Максимальное ожидание условия в $wait(...). По умолчанию 30 сек.")
      end
      ...
      if imgui.Button(" Сброс к дефолту") then
        custom_vars = {}
        for k, v in pairs(builtin_custom_vars) do custom_vars[k] = v end
    ...
    if imgui.BeginTabItem("Переменные") then
      ...
      imgui.Text("Имя"); imgui.NextColumn()
      imgui.Text("Значение"); imgui.NextColumn()
      ...
      if edit_key then imgui.OpenPopup("Переименование переменной") end
      if imgui.BeginPopupModal("Переименование переменной", nil, imgui.WindowFlags.AlwaysAutoResize) then
      ...
      -- Попап: удаление
      if del_key then imgui.OpenPopup("Удалить переменную?") end
      if imgui.BeginPopupModal("Удалить переменную?", nil, imgui.WindowFlags.AlwaysAutoResize) then
    ...
    if imgui.BeginTabItem("Импорт/экспорт") then
      imgui.TextWrapped("Экспортирует/импортирует только пользовательские переменные и настройки.")
...
imgui.OnFrame(function() return showTagsWindow[0] end, function()
  ...
  imgui.TextColored(imgui.ImVec4(0.75, 1, 1, 1), "Подставляй теги — получай готовый текст")
  imgui.Separator()
