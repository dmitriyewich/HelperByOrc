...
function module.Hint(str_id, hint, delay)
    ...
    if show then
        ...
        if elapsedTime <= animTime then
            alpha = hovered and math.max(0, math.min(1, elapsedTime / animTime)) or math.max(0, math.min(1, 1 - elapsedTime / animTime))
            imgui.PushStyleVarFloat(imgui.StyleVar.Alpha, alpha)
...
function module.customVerticalMenu(items, current)
    ...
    for i, item in ipairs(items) do
        ...
        -- Анимация альфы
        state.alpha = state.alpha + (target_alpha - state.alpha) * math.min(imgui.GetIO().DeltaTime * anim_alpha_speed, 1)
        -- Анимация сдвига
        state.shift = state.shift + (target_shift - state.shift) * math.min(imgui.GetIO().DeltaTime * anim_shift_speed, 1)
        menu_animations[i] = state

        -- Фон с анимацией
        if state.alpha > 0.01 then
            local col = is_selected and imgui.ImVec4(
                color_selected.x, color_selected.y, color_selected.z, color_selected.w * state.alpha
            ) or imgui.ImVec4(
                color_hovered.x, color_hovered.y, color_hovered.z, color_hovered.w * state.alpha
            )
...
-- Create state table for dragging (stores flag, offset and last position)
function module.newDragState()
    return {
        dragging   = false,
        dragOffset = imgui.ImVec2(0, 0),
        lastPos    = imgui.ImVec2(0, 0)
    }
end
...
function module.handleWindowDrag(state)
    ...
    if not state.dragging
        and imgui.IsWindowHovered(imgui.HoveredFlags.RootAndChildWindows)
    ...
    if state.dragging and imgui.IsMouseDown(0) then
        ...
        imgui.SetWindowPosVec2(newPos, imgui.Cond.Always)
    else
        state.dragging = false
        state.lastPos = (state.lastPos.x ~= 0 or state.lastPos.y ~= 0) and state.lastPos or winPos
        module.clampToScreen(state.lastPos)
        imgui.SetWindowPosVec2(state.lastPos, imgui.Cond.Always)
    end
