...
function module.TextColoredRGB(text, align, aBool)
    ...
    local render_text = function(text_)
        for w in string.gmatch(text_, "[^\r\n]+") do
            ...
            elseif align == 2 then
                imgui.SetCursorPosX(
                    imgui.GetCursorPosX() + width - text_width.x - imgui.GetScrollX() -
...
function module.Hint(str_id, hint, delay)
    ...
    if show then
        ...
        if elapsedTime <= animTime then
            alpha =
                hovered and math.max(0, math.min(1, elapsedTime / animTime)) or
                math.max(0, math.min(1, 1 - elapsedTime / animTime))
            imgui.PushStyleVarFloat(imgui.StyleVar.Alpha, alpha)
...
function module.customVerticalMenu(items, current)
    ...
    for i, item in ipairs(items) do
        ...
        -- Анимация альфы
        state.alpha =
            state.alpha + (target_alpha - state.alpha) * math.min(imgui.GetIO().DeltaTime * anim_alpha_speed, 1)
        -- Анимация сдвига
        state.shift =
            state.shift + (target_shift - state.shift) * math.min(imgui.GetIO().DeltaTime * anim_shift_speed, 1)
        ...
        if state.alpha > 0.01 then
            local col =
                is_selected and
                imgui.ImVec4(color_selected.x, color_selected.y, color_selected.z, color_selected.w * state.alpha) or
                imgui.ImVec4(color_hovered.x, color_hovered.y, color_hovered.z, color_hovered.w * state.alpha)
