...
- HUD-конструктор экранных виджетов: inline-текст или заметка Блокнота, разметка, картинки, переменные, пресеты и размещение мышью поверх игры; обычные виджеты не перехватывают ввод и не мешают быстрому меню.
...
5. Настройте язык, масштаб, уведомления, хоткеи, быстрое меню и профиль во вкладке «Настройки». Уровень лога и пути находятся в разделе «Диагностика».
...
HUD v1 не импортирует старые HUD-конфиги. Картинки в разметке читаются только из профильных папок, URL, абсолютные пути и `..` блокируются. Условия HUD используют общий список блокировок Биндера; `refresh_ms=0` обновляет виджет каждый кадр и может снизить FPS.
