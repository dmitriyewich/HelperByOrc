...
- HUD-конструктор экранных виджетов: inline-текст или заметка Блокнота, разметка, картинки, переменные, расширенный стиль окна (цвета, рамка, тень, межстрочные отступы), пресеты и размещение мышью поверх игры; обычные виджеты рисуются нижним слоем под окнами Helper и не перехватывают ввод.
...
HUD v1 не импортирует старые HUD-конфиги. Картинки в разметке читаются только из профильных папок, URL, абсолютные пути и `..` блокируются. Стиль виджета хранит per-widget цвета, отступы окна, интервалы строк через `item_spacing_y`, рамку, тень и разделители. Условия HUD используют общий список блокировок Биндера; старые флаги скрытия чата/диалога мигрируют в обычные условия, а скрытие при открытом Helper больше не применяется. `refresh_ms=0` обновляет виджет каждый кадр и может снизить FPS.
