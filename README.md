...
- Помощник текстового триггера позволяет выбрать точную RakNet-строку из последних 100 сообщений, автоматически находит изменяемые части и применяет PCRE2-шаблон только после подтверждения; нужные значения доступны через `[chatwordsex(...)]`.
- Категории, папки, поиск, ручная сортировка, два вида списка (`Проводник` и `Две панели`) и отдельные настройки сохранения последней папки и состояния раскрытия между запусками.
...
- `[waitif(...)]` неблокирующе ждёт состояние игры, игрока, транспорта или клавиши и продолжает текущий бинд.
- Ручной выбор цели для `{target...}`: `Q` включает курсор, а видимый игрок под ним выделяется белой контурной обводкой; дальность и фильтр по видимому нику SA:MP настраиваются.
...
- Фильтр сообщений с понятным списком правил, автоматическим помощником, живой проверкой и выбором строки из общей истории последних 100 RakNet-сообщений.
...
Поддержка привязана к проверенным бинарным вариантам. Неизвестный SHA-256,
включая DLL с известным entry point, отключает все зависящие от адресов функции
и SA:MP/RakNet hooks.
...
4. Выберите профиль и настройте язык, масштаб, горячие клавиши и быстрое меню. Ручной выбор цели для `{targetid}` включён по умолчанию: нажмите `Q`, наведите курсор на игрока и нажмите ЛКМ. В `Настройки -> Горячие клавиши` меняются клавиша, дальность выбора и необязательный фильтр по видимому нику SA:MP.
