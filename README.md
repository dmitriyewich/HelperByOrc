...
- Стабильный cursor ownership для ImGui: главное окно берёт SA:MP cursor mode только на время интерактивного UI; quick menu использует input routing без `LOCKCAMANDCONTROL`, поэтому игрок не блокируется, но пункты остаются кликабельными поверх SA:MP chat/dialog и уступают видимому Arizona CEF или чужому cursor/input owner.
