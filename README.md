...
- Стабильный cursor ownership для ImGui: главное окно берёт SA:MP cursor mode только на время интерактивного UI; quick menu показывает видимый курсор через `CURSOR_LOCKCAM` без `LOCKCAMANDCONTROL`, пункты остаются кликабельными поверх SA:MP chat/dialog и уступают видимому Arizona CEF или чужому cursor/input owner.
...
- HUD v2: единый профессиональный редактор экранных виджетов прямо во вкладке HUD: рабочая область с главным холстом, компактная панель действий по группам, адаптивные панели, ручной hit-test без конфликтующих overlay-кнопок, крупные ручки размера, рамка выделения, перемещение view средней кнопкой, масштаб, мягкий магнит к сетке, слои, инспектор секциями `Основное`/данные элемента/`Стиль`/`Видимость`/`Дополнительно`, режимы текста, picker переменных, импорт картинок, lock/hide, group/ungroup, undo/redo через toolbar, пресеты и импорт/экспорт `.helperhud.json`; обычные виджеты рисуются нижним слоем под окнами Helper и не перехватывают ввод.
