...
#define APP_UI_TEXTS(X) \
    ...
    X(TagsBuiltinDialogGetIdDescription, "Возвращает ID активного диалога SA:MP. Также доступен legacy-алиас {getdialogid}. Если диалог не открыт, возвращает пустую строку.", "Returns the ID of the active SA:MP dialog. The legacy alias {getdialogid} is also available. If no dialog is open, returns an empty string.") \
    ...
    X(TagsBuiltinSaveDialogDescription, "Сохраняет активный диалог в .txt файл в папку Documents\\\\GTA San Andreas User Files\\\\HelperByOrc\\\\saved\\\\dialogs. Если аргумент не указан, имя файла берётся из заголовка диалога. Ничего не вставляет в текст.", "Saves the active dialog to a .txt file in Documents\\\\GTA San Andreas User Files\\\\HelperByOrc\\\\saved\\\\dialogs. If no argument is provided, the file name is derived from the dialog caption. Inserts no text.") \
    ...
    X(ToastSaveDialogDocumentsUnavailable, "Не удалось определить путь к Documents для [save_dialog(...)].", "Failed to resolve the Documents path for [save_dialog(...)]") \
